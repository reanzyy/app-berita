<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('My Posts') }} &raquo {{ __('Create Post') }}
        </h2>
    </x-slot>
                    ...
                    <form action="{{ route('posts.store') }}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="news_content"
                                class="@error('news_content') bg-red-50 border border-red-500 text-red-900 placeholder-red-700 text-sm rounded-lg focus:ring-red-500 focus:border-red-500 block w-full p-2.5 @else border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 @enderror">{!! old('news_content') !!}</textarea>
                            @error('news_content')
                                <div class="text-red-600">{{ $message }}</div>
                            @enderror
                        </div>
